...
const DB = {
  ...
  updateUser: (userId, updates) => {
    ...
    if (idx !== -1) {
      ...
      // Supabaseが初期化されている場合はPostgreSQL側のテーブル(public.users)も更新する
  ...
  deleteUser: (userId) => {
    ...
    // Supabaseが有効な場合のアカウント連携削除
    if (window.supabaseClient) {
      if (Auth.currentUser && Auth.currentUser.id === userId) {
        // 自身の退会処理
        window.supabaseClient.rpc('delete_me').then(({ error }) => {
          if (error) console.error("Supabase delete_me error:", error);
        });
      } else if (Auth.currentUser && Auth.currentUser.role === 'admin') {
        // 管理者による他ユーザーの強制削除
        window.supabaseClient.rpc('delete_user_by_admin', { target_user_id: userId }).then(({ error }) => {
          if (error) console.error("Supabase delete_user_by_admin error:", error);
        });
      }
    }
    ...
    articles.forEach(a => {
      if (a.likedBy) {
        a.likedBy = a.likedBy.filter(uid => uid !== userId);
        a.likes = a.likedBy.length;
      }
      if (a.viewedBy) {
        a.viewedBy = a.viewedBy.filter(uid => uid !== userId);
      }
      if (a.comments) {
        a.comments = a.comments.filter(c => c.userId !== userId);
      }
    });
    DB.saveArticles(articles);
