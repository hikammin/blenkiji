...
Pages.ArticleCreate = (editId) => {
  ...
  window.toggleMarkdownHint = () => {
    const box = document.getElementById('markdown-hint-box');
    const btn = document.getElementById('hint-toggle-btn');
    if (!box) return;
    const isHidden = box.style.display === 'none';
    box.style.display = isHidden ? 'block' : 'none';
    if (btn) btn.style.color = isHidden ? 'var(--primary-light)' : 'var(--text-muted)';
  };
        ...
        <form id="create-form" onsubmit="handleCreateSubmit(event)">
          ...
          <div id="steps-container">
            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
              <h2>コンテンツ</h2>
              <button type="button" id="hint-toggle-btn" onclick="toggleMarkdownHint()" style="font-size: 0.78rem; padding: 0.25rem 0.75rem; background: rgba(255,255,255,0.07); border: 1px solid var(--border-glass); border-radius: 20px; color: var(--text-muted); cursor: pointer; transition: all 0.2s;">💡 書き方のヒント</button>
            </div>
            <div id="markdown-hint-box" style="display:none; padding: 1rem; background: rgba(255,255,255,0.05); border-radius: var(--radius); border-left: 4px solid var(--primary); margin-bottom: 1.25rem; animation: fadeInUp 0.25s ease;">
              <div style="display:flex; justify-content: space-between; align-items: flex-start;">
                <div style="font-size: 0.88rem; color: var(--text-main); line-height: 1.9;">
                  <strong>💡 書き方ヒント（Markdown対応）</strong><br>
                  <code style="background:rgba(255,255,255,0.1);padding:0.1rem 0.35rem;border-radius:4px;font-family:var(--font-mono);color:var(--primary-light);">\`G\`</code> → ショートカットキーとして強調表示<br>
                  <code style="background:rgba(255,255,255,0.1);padding:0.1rem 0.35rem;border-radius:4px;font-family:var(--font-mono);color:var(--primary-light);">**テキスト**</code> → <strong>太字</strong> に表示<br>
                  <span style="font-size:0.8rem; color: var(--text-muted);">バッククォートは Shift+@ で入力できます。</span>
                </div>
                <button type="button" onclick="toggleMarkdownHint()" style="background:none; border:none; color:var(--text-muted); font-size:1.2rem; cursor:pointer; padding: 0 0.25rem; line-height:1; flex-shrink:0;">✕</button>
              </div>
            </div>
