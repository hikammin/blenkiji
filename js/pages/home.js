...
Pages.Home = () => {
  ...
  const renderWeeklyTop = () => {
    if (weeklyTop.length === 0) {
      return '<p class="text-center" style="color: var(--text-muted);">今週はまだ記事がありません。</p>';
    }
    return `
      <div class="grid md:grid-cols-3">
        ${weeklyTop.map(a => Components.ArticleCard(a)).join('')}
      </div>
    `;
  };
    ...
    <main class="main-content">
      <div class="container">
        ...
        <section class="mb-8">
          <div class="flex items-center justify-between mb-4">
            <h2>今週のおすすめ</h2>
            <span style="font-size: 0.8rem; color: var(--text-muted);">いいね・閲覧数の多い順 (今週)</span>
          </div>
          ${renderWeeklyTop()}
        </section>
